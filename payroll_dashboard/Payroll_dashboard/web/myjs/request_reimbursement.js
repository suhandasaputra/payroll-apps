...
function loaddata() {
    ...
    $('#table_requestreimbursement').dataTable().fnClearTable();
    $('#table_requestreimbursement').dataTable().fnDestroy();
    ...
    $.ajax({
        ...
        dataType: "json",
        url: "RequestReimbursement",
        data: JSON.stringify(datjson),
        type: 'POST',
        success: function (response) {
            if (response.resp_code == 0000) {
                var datauser = response.list;
                $('#table_requestreimbursement').DataTable({
                    "scrollX": true,
                    "data": datauser,
                    "columns": [
                        {data: "reference",
                    ...
                    "createdRow": function (row, data, dataIndex) {
                        ...
                        if (data.approve_status_head == 'APPROVED') {
                            $(row).addClass('green');
                        } else if (data.approve_status_head == 'REJECTED') {
                            $(row).addClass('red');
                        } else if (data.approve_status_admin == 'APPROVED') {
                            $(row).addClass('yellow');
                        } else if (data.approve_status_admin == 'REJECTED') {
                            $(row).addClass('red');
                        } else if (data.approve_status_admin == 'PENDING') {
                            $(row).addClass('yellow');
                        }
                    },
                    dom: 'Bfrtip',
