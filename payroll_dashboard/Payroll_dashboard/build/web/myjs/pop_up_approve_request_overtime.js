...
$('#table_requestovertime').on('click', 'td', function () {
    var $row = $(this).closest("tr");
//    if ($row.find(".absence_type").text() == 'REQ') {
    ...
    $.ajax({
        ...
        success: function (resp) {
            if (resp.resp_code == 0000) {
                ...
                document.getElementById("overtime_date_detail_request").value = resp.overtime_date;
                document.getElementById("date_request_detail_request").value = resp.date_request;

                document.getElementById("schedule_in_detail_request").value = resp.schedule_in;
                document.getElementById("schedule_out_detail_request").value = resp.schedule_out;
                document.getElementById("overtime_before_detail_request").value = resp.overtime_before;
                document.getElementById("overtime_after_detail_request").value = resp.overtime_after;


                document.getElementById("overtime_payload_detail_request").value = resp.overtime_payload;
                document.getElementById("overtime_all_detail_request").value = resp.overtime_all;
    ...
    if (reference == "") {
        ...
        if (employee_id == "") {
            ...
            if (level_desc !== 'head') {
                $.ajax({
                    dataType: "json",
                    url: "RequestOvertimeApproveAdmin",
                    data: JSON.stringify(datjson),
                    type: 'post',
                ...
                $.ajax({
                    dataType: "json",
                    url: "RequestOvertimeApproveHead",
    ...
    if (reference === "") {
        ...
        if (employee_id === "") {
            ...
            if (level_desc !== 'head') {
                $.ajax({
                    dataType: "json",
                    url: "RequestOvertimeRejectAdmin",
                    data: JSON.stringify(datjson),
                    type: 'post',
                    success: function (response) {
                        if (response.resp_code == 0000) {
