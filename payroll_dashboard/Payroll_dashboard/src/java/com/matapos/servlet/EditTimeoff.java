 ...
 * @author azzam
 */
@WebServlet(name = "EditTimeoff", urlPatterns = {"/EditTimeoff"})
public class EditTimeoff extends HttpServlet {
    ...
    protected void processRequest(HttpServletRequest request, HttpServletResponse response)
        ...
        try (PrintWriter out = response.getWriter()) {
            ...
            out.println("<html>");
            out.println("<head>");
            out.println("<title>Servlet EditEmployeeStatus</title>");
            out.println("</head>");
            out.println("<body>");
            out.println("<h1>Servlet EditEmployeeStatus at " + request.getContextPath() + "</h1>");
            out.println("</body>");
            out.println("</html>");
        }
    }
...
//        MessageProcess mp = new MessageProcessImpl();
        ...
        try {
            ...
            String timeoff_code = newJson.get("timeoff_code").toString();
            String timeoff_desc = newJson.get("timeoff_desc").toString();
            String absence_type = newJson.get("absence_type").toString();
            String bucket_cuti = newJson.get("bucket_cuti").toString();

            req.put(FieldParameterMatapos.proccode, ProcessingCode.edittimeoff);
            req.put("company_id", company_id);
            req.put("user_id", user_id);
            req.put("timeoff_code", timeoff_code);
            req.put("timeoff_desc", timeoff_desc);
            req.put("absence_type", absence_type);
            req.put("bucket_cuti", bucket_cuti);
